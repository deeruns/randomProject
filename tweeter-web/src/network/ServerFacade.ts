...
import { StatusDto } from "tweeter-shared";
  ...
  export class ServerFacade {
    private SERVER_URL = "https://u3ltdnmf7f.execute-api.us-east-1.amazonaws.com/dev";
   ...
   public async loadFeedItems(request: FeedItemsRequest):
    ...
    public async Register(request: RegisterRequest):
        ...
        const requestWithArray = {
            ...request,
            userImageBytes: Array.from(request.userImageBytes), // Convert Uint8Array to array
        };
        const response = await this.clientCommunicator.doPost<
            RegisterRequest,
            RegisterResponse
            >(requestWithArray, "/auth/register");
        //return user object from dto
        const user: User | null = response.success && response.user ?
            User.fromDto(response.user) as User : null;
