  ...
  public async Follow(request: followRequest):
   ...
   public async loadFeedItems(request: FeedItemsRequest):
    ...
    Promise<[Status[], boolean]> {
        ...
        //DO I NEED TO MAP THE SEGMENTS HERE? OR IS IT FINE TO LEAVE THE POST SEGMENT DTO OBJECT?
        const feed: Status[] | null = (response.success && response.statuses)
            ? response.statuses.map(dto => Status.fromDto(dto) as Status)
            : null;


        const segments: PostSegment[] | null = (response.success && response.statuses)
            ? response.statuses.flatMap(dto => dto.segments).map(dto => PostSegment.fromDto(dto) as PostSegment)
            : null;
